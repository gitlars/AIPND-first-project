...
def calculates_results_stats(results_dic):
    ...
    for key in results_dic:
        if results_dic[key][2]: # labels match
            n_match += 1
            if results_dic[key][3]: # label is-a-dog
                n_correct_breed += 1
            
        if results_dic[key][3]: # label is-a-dog
            n_dogs_img += 1
            if results_dic[key][4]: # classified is-a-dog
                n_correct_dogs += 1
                
        else:
            n_notdogs_img += 1
            if results_dic[key][4] == 0: # classified is-NOT-a-dog
                n_correct_notdogs += 1

    # Load up dictionary keys
    ...
    results_stats_dic["pct_match"] = 100. * n_match / n_images
    results_stats_dic["pct_correct_dogs"] = 100. * n_correct_dogs / n_dogs_img
    results_stats_dic["pct_correct_breed"] = 100. * n_correct_breed / n_dogs_img
    if n_notdogs_img > 0:
        results_stats_dic["pct_correct_notdogs"] = 100. * n_correct_notdogs / n_notdogs_img
    else:
        results_stats_dic["pct_correct_notdogs"] = 0.
    
    #print("Pct Corr overall: {}".format(results_stats_dic["pct_match"]))
