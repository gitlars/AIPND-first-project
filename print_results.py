...
def print_results(results_dic, results_stats_dic, model, 
    ...
    print("Number of NOT-a-Dog Images: {}\n".format(n_notdogs_img))

    print("% Correct Is-a-Dog: {}".format(pct_correct_dogs))
    print("% Correct NOT-a-Dog: {}".format(pct_correct_notdogs))
    print("% Correct Breed: {}".format(pct_correct_breed))
    print("% Correct Matches: {}".format(pct_match))
        
    # If asked to print the incorrectly classified images vis-a-vis is-a-dog-edness
    # (and if there are any)
    if print_incorrect_dogs and ((n_correct_dogs + n_correct_notdogs) != n_images):
        print("Incorrect Is-a-Dog Classifications:")
        for key in results_dic:
            # if labeled is-a-dog doesn't match classified is-a-dog
            if results_dic[key][3] != results_dic[key][4]:
                print("Image {:>28} was labeled {:>20} but classified as {:>30}".format(
                    key, results_dic[key][0], results_dic[key][1]))
    
    # If asked to print the incorrectly classified dog images vis-a-vis breed
    # (and if there are any)
    if print_incorrect_breed and (n_correct_dogs != n_correct_breed):
        print("\nIncorrect Dog Breed Classifications:")
        for key in results_dic:
            # if pet image is labeled is-a-dog but classified doesn't match label
            if (results_dic[key][3] > 0) and (results_dic[key][2] == 0):
                print("Dog image {:>28} was labeled {:>20} but classified as {:>30}".format(
                    key, results_dic[key][0], results_dic[key][1]))
